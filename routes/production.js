...
async function interpretData(crop, requestId) {
    ...
    try {
      ...
      const pdfUrl = `./pdfs/${requestId}.pdf`;
      doc.pipe(fs.createWriteStream(pdfPath));
      doc.fontSize(12).text(completion.data.choices[0].message.content , { align: 'justify' });
...
//route to create the request fir data from open ai
...
router.get('/:requestId', async (req, res) => {
  try {
    ...
    if (response.pdfUrl) {
      res.json({ pdfUrl: response.pdfUrl });
    } else {
      res.status(204).send();
    }
  } catch (error) {
